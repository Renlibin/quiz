...
public class UserDaoExt extends UserDao {
	...
	public Integer insertWithReturning(User user){
		
		logger.info("New user {} is going to be persisted");
		
		UserRecord record =context.insertInto(USER, USER.NAME, USER.NICKNAME, 
		...
		if(logger.isDebugEnabled()){
			logger.debug("The new user id is {}", record.getId());
		}
